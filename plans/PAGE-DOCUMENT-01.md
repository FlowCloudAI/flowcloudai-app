...
- M6 隔离画布：实现完成；macOS、Android 原生验收通过，Windows 待验证（55dc268、ff43dbf、6f97233、823ae5a、f66862c、475ab8a、25115db、7eef3b9；后续原生证据与修正见对应提交）。独立打包页面叠加严格 CSP，iframe 仅开放 `allow-scripts`；画布运行时构建为唯一的内联 IIFE，并以精确 SHA-256 同时满足画布 meta CSP 与 Tauri 页面专属响应头。macOS 调试构建已先后发现并修正 opaque-origin 下外链资源被 CSP 拒绝、内联脚本早于根节点启动、未分层运行时默认值压过作者主题三个问题。用户随后在 macOS 开关构建和 Android aarch64 debug APK 上确认合法样例与行内样式正确，两道校验分别阻断恶意样例，Network 面板无外部请求，链接不造成白屏或顶层跳转，画布高度稳定跟随内容，后台恢复后仍可继续渲染，当前词条显示标题与 Markdown 降级正文；两端的 `window.origin === "null"` 与跨文档访问 `SecurityError` 也确认了 opaque origin。带 token、来源 Window、版本、序号和大小上限的 bridge 已接入只读渲染、选择、高亮、根内容尺寸、导航意图及受限纯文本输入。宿主正常路径经 `previewCompiler` 与 guard 后下发，显式探针绕过时仍由画布独立校验。桌面与移动查看态预览、页面编辑器和画布制品现在默认进入构建；共享恶意样例与绕过按钮改由 `VITE_PAGE_DOCUMENT_PROBE=1` 单独开启。Vite 开发模式也通过前置中间件直接返回同一制品模块组装的单内联脚本与精确哈希 CSP，不经 Vite HTML 变换或 HMR 注入。`npm run test:page-document` 当前覆盖 64 个文件、382 项通过；Windows WebView2 的基础隔离行为以及三端的第四批 4a/4b 输入行为仍待原生产物验收。
- M7 桌面编辑页：第四批 4a、4b 的代码与自动测试完成，三端原生输入验收待执行。第一批已由用户在 macOS 开关调试构建中通过七项人工验收：进入页面模式、代码编辑保存与重开、中文输入、危险内容拦截、离开确认与切标签不弹、Markdown 不受影响、浏览模式预览 revision。第三批又由用户在 macOS 通过七项人工验收：组件树与返回分类、键盘导航、滑杆与快捷值调属性、四边联动与清除、颜色、旧段落纳入、三项回归。第二、三批已接入画布与左栏双向选择、固定组件树、专用 SVG 图标、结构化属性调节、140ms 连续属性提交，以及逐个纳入可安全推断类型的不透明元素。第四批 4a 给 paragraph、heading、list-item、table-cell 四类既有受管节点开放可信命令授予的 `plaintext-only` 编辑，组合期不提交也不应用宿主渲染；4b 增加 paragraph、heading、list-item 的 Enter 分段、块内软换行和纯文本粘贴，空行按一次内核提交拆块，表格单元格拒绝分段。新块 UUID 只由宿主分配，输入意图仍只含节点 ID、UTF-16 区间、expected 与纯文本，经当前投影复核后进入同一草稿、历史与保存链路；作者 `contenteditable` 仍被隔离层拒绝。页面预览、编辑入口和 CodeMirror 现在默认进入产物，CodeMirror 仍保持独立分块；隔离探针仅在 `VITE_PAGE_DOCUMENT_PROBE=1` 下进入。后续顺序确定为：移动端页面编辑 → 图片与内链入口 → 备份后重置旧正文并改接 AI 与搜索 → 删除 Markdown。旧正文会先备份再重置；Markdown 入口等页面模式达到基本可写后再删除。仍未实现拖放、跨节点删除与合并、画布内撤销快捷键、行内格式、链接编辑、表格/列表结构操作、分宽度编辑、AI 分屏、拖拽与 Grid 手柄、移动端专用 caret reveal、项目模板编辑与项目模板存储。Windows WebView2 的基础隔离以及 macOS、Android、Windows 的 4a/4b 输入与输入法仍待原生验证。`npm run test:page-document` 覆盖 64 个文件、382 项通过，Dock 布局 9 项通过。
