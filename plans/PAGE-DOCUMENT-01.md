...
本计划采用移植式重写，覆盖 0.2.0 阶段 A–C 中与页面文档有关的部分。唯一上游输入是实验仓的[可行性结论与移交清单](../../app_entry_page_editor/docs/可行性结论与移交清单.md)，对应代码快照 `6e73b02`；实验已经结束，已有内核、引擎和测试可逐模块作为草稿移入，随后按本仓目录、命名、token 和平台边界重组，后续实现归 `app_main` 所有。桌面词条正文现已切换到页面文档；移动端仍保留 Markdown，原生编辑验收也未闭环。后续实现不引用实验仓源码、不建子模块、不写同步脚本，代价是本仓独立维护实现与回归证据。
...
生产数据布局先写数据 ADR，再由词条与项目首页两条切片决定。当前 core 只有词条字符串与现有 SQL 事务，缺源码修订、CAS 和首页文档；先在 `app_main/docs/adr/` 记录生产语义及 core 落点，比较领域对象分别持源码、明确关联的页面记录等方案，在 `core_world_data` 建 HTML/CSS、revision、原子保存和派生索引最小模型。源码仍按领域对象分表，不建万能文档源码表，不为未来种类建空表；身份、引用、段落级派生索引、删除记录、变更记录与保存回执按 [ADR 0004](../docs/adr/0004-unified-object-layer-and-asset-scope.md) 统一为一层。代价是将来真实闭环推翻布局时仍须迁移并修订 ADR。
...
| 派生边界 | 从已校验 HTML 生成段落块（节点 ID 与纯文本）和引用集合，写入统一对象层（ADR 0004 §3）；提供重建操作；资料字段保持结构化 | 源码摘要、revision、段落块与引用重建差分；保存失败及查询回归 | 派生重建不改源码/revision；段落块可按节点 ID 定位；预览 DOM 和搜索结果均不能反写正文 |
...
同步准备只落在字段与边界，不实现同步系统。按工作区 `plans/0.5.0规划.md` §6 与 `plans/0.2.0规划.md` §2.5、§2.6，下面各项随 A2/A3 的模型落位；字段名为 ADR 候选，语义必须明确，不能只写“预留扩展”。

| 同步接缝 | 字段/边界落点 | 最小证明 |
| --- | --- | --- |
| 修订与 CAS | 页面记录 `revision` 单调增加，保存请求 `expected_revision`；联合保存为每个被写目标携带期望值 | 冲突整批拒绝，无最后写覆盖 |
| 修改者 | 修订记录 `modified_by` 指向本地用户主体；变更记录另存修改来源（用户、AI、插件、合并、导入） | UI/源码/AI 都记录本地主体，AI 来源作为修改来源记录但不冒充账号 |
| 删除墓碑 | 删除经领域命令在对象登记层标记删除修订、`deleted_at` 与发起操作的主体；级联删除同事务逐一记录；触发器只校验登记已标记，不代写 `system`（ADR 0004 §4） | 删词条/项目后仍有可重放删除记录，删除者为发起操作的本地主体，不只硬删除 |
| 资产稳定 ID | 资产记录 `asset_id`，引用关系保存 ID；摘要、处理版本与原件/派生关系独立 | 重定位、重建预览不改变被引用资产身份 |
| 空间归属 | 项目记录 `space_id` 指向本地默认空间 | 创建、复制/导入时显式赋值，不引入成员或账号功能 |
| 统一对象层 | 对象登记、引用、段落块、变更记录与保存回执按对象种类统一，种类不写数据库 `CHECK`（ADR 0004 §3） | 新增一个测试种类无需修改表结构；未知种类按不透明对象保留 |
| 节点身份 | `data-fc-node-id` 的保持与重映射规则（ADR 0004 §5） | 复制粘贴、拆分合并、AI 候选、转为本地组件的节点 ID 自动化测试 |

### A3 资产逻辑 ID 往返

资产身份必须贯穿数据库、Tauri 协议、Android WebView 和离线导出。当前路径及文件名推导身份不能支撑重定位；建立最小资产记录和解析适配，导入一份原件后以逻辑 ID 写入 HTML/CSS，宿主按文档授权解析为临时显示资源，并通过离线导出相对资源引用重开。这里仅验证接口与身份，不展开 B 的图像处理队列；代价是需要把项目归属、资源准备和文档引用提交分开管理。资产 ID 全局唯一，归属范围单独记录（ADR 0004 §6）：资产记录只保存身份与内容事实，项目、空间、组件库与插件包作为范围登记；宿主沿「文档 → 组件实例 → 组件定义 → 资产」引用链授权解析。当前实现把资产非空绑定项目，并以 `CHECK` 限定媒体类型，须在 A 退出前迁移。

| 做法 | 证据形式 | 退出条件 |
| --- | --- | --- |
| 数据库记录稳定 ID、内容摘要、原件/派生关系和处理版本；准备资源成功后才允许文档引用提交；失败不产生已引用的缺失资产 | 资产和事务测试、失败注入对照 | 原件不被预览覆盖，未引用准备资源可清理，重建派生表示不改逻辑 ID |
| 桌面/Android 使用受管 Tauri 协议加载，检查不存在、错项目或错范围、引用链中断、遍历/编码绕过和越权 ID | 协议自动化测试、三种 WebView 的合法资源与拒绝记录 | 可显示合法图片，非法请求失败；HTML/CSS、导出包和 bridge 都不泄露本机路径 |
...
| 冻结对象 | 冻结内容 | 不在 A 冻结 |
| --- | --- | --- |
| 页面文档 | HTML/CSS 同草稿；受控引用（组件实例、事实绑定、资产、对象内链）及其解析与缺失降级；组件实例序列化格式；稳定节点身份及其保持与重映射规则；revision 保存前置条件；原子保存；安全编译与派生索引边界 | 万能文档源码表、未来对象种类全集；块集合、HTML/CSS 能力策略、断点模型均在阶段 C 验收前可调 |
| 资产 | 稳定身份；身份与归属范围分离、引用链授权；原件不被预览覆盖；派生表示可重建；内容摘要与处理版本参与缓存 | 视频/音频字段全集、代理与时间轴、大文件打包策略 |
...
旧 Markdown 数据按用户新决定走临时转换层，不再执行“备份后重置”。转换在桌面打开时只生成草稿，绝不改写 `entries.content` 或自动保存；用户显式保存后形成 revision 1 页面文档。标题、段落、列表、旧图片引用及 UUID/标题内链按有限子集转换，其余语法整段转义保留原文。正式上线不支持旧格式；只有旧数据全部至少完成一次页面文档保存并核对结果后，才能删除转换层及其调用点，不设双写或长期兼容。
...
| 旧 Markdown 开发数据 | 桌面入口切换及正式上线前 | 临时转换层生成页面草稿；用户保存才迁移 | 已否定“备份后重置”。不改旧正文、不自动保存；完成所有旧数据的一次保存并核对后删除转换层，正式上线不支持旧格式。移动端迁移单独验收 |
...
| 统一对象层与资产范围迁移 | A2、A3 退出 | 一次迁移 0010/0011 全部数据；按对象种类分批迁移 | 建议在 A 退出前一次完成，保护开发数据并通过两条特性测试链路；分批会让新旧两套墓碑与回执并存 |
| 画布 `@layer` 与 macOS 最低版本 | A1 退出 | 已选择把打包配置最低版本提至 12.3，画布运行时固定七层顺序；宿主缺少 `CSSLayerBlockRule` 时关闭编辑入口并提示升级 | 仍需在 macOS 打包产物核验最低版本、在旧 Android WebView 验收提示；不以构建通过代替原生验收 |

块集合和断点模型在 C 验收前保持可调，不另设无依据的提前决策。建议从实验的移动基础/桌面覆盖两档及基础块开始，以非专业任务和原生样例验证；需要改变时同步能力卡、fixture、保存/导出版本规则。组件实例的序列化格式与缺失降级属于页面文档不变量，须在 A 冻结前定稿（ADR 0004 §1）；组件编辑模式、影响范围提示、转为本地组件的界面、阶段 E 容器字节布局和后续高级能力不阻塞本计划的 A/C，不列为本批待决事项。
...
- M4 文档内核：移植完成（ca78378、aeee508）。当前 `npm run test:page-document` 随编辑页第四批 4b 回归一起明确覆盖全部 64 个测试文件，共 382 项通过；领域导入闭包由测试约束为不依赖 React、Tauri、DOM 全局及反向业务模块。
- M5 草稿会话：部分接入（ca78378、4750201、8079839）。词条作用域的源码编辑、前端防抖校验、独立保存、失败重试、revision 冲突、放弃、撤销/重做和可视属性候选已接到同一生产 hook；影响其他规则的可视修改会先确认。AI 与整份候选预览确认仍未接入。本仓尚无项目模板存储，项目作用域当前只用 `CANVAS_BASE_PROJECT_HTML/CSS` 作为只读编译基线，不提供编辑或保存。
- M6 隔离画布：实现完成；macOS、Android 原生验收通过，Windows 待验证（55dc268、ff43dbf、6f97233、823ae5a、f66862c、475ab8a、25115db、7eef3b9；后续原生证据与修正见对应提交）。独立打包页面叠加严格 CSP，iframe 仅开放 `allow-scripts`；画布运行时构建为唯一的内联 IIFE，并以精确 SHA-256 同时满足画布 meta CSP 与 Tauri 页面专属响应头。macOS 调试构建已先后发现并修正 opaque-origin 下外链资源被 CSP 拒绝、内联脚本早于根节点启动、未分层运行时默认值压过作者主题三个问题。用户随后在 macOS 开关构建和 Android aarch64 debug APK 上确认合法样例与行内样式正确，两道校验分别阻断恶意样例，Network 面板无外部请求，链接不造成白屏或顶层跳转，画布高度稳定跟随内容，后台恢复后仍可继续渲染，当前词条显示标题与 Markdown 降级正文；两端的 `window.origin === "null"` 与跨文档访问 `SecurityError` 也确认了 opaque origin。带 token、来源 Window、版本、序号和大小上限的 bridge 已接入只读渲染、选择、高亮、根内容尺寸、导航意图及受限纯文本输入。宿主正常路径经 `previewCompiler` 与 guard 后下发，显式探针绕过时仍由画布独立校验。桌面与移动查看态预览、页面编辑器和画布制品现在默认进入构建；共享恶意样例与绕过按钮改由 `VITE_PAGE_DOCUMENT_PROBE=1` 单独开启。Vite 开发模式也通过前置中间件直接返回同一制品模块组装的单内联脚本与精确哈希 CSP，不经 Vite HTML 变换或 HMR 注入。`npm run test:page-document` 在第四批 4b 后覆盖 64 个文件、382 项通过；Windows WebView2 的基础隔离行为以及三端的第四批 4a/4b 输入行为仍待原生产物验收。
- M7 桌面编辑页：页面编辑、旧 Markdown 临时转换、桌面唯一页面正文入口及 AI 旧正文写入护栏已接入；第三批七项 macOS 人工验收已通过，第四批输入与输入法仍待三端原生验收。双链候选已由用户完成桌面人工验收。图片入口与项目范围授权已接入同一草稿、撤销和 revision 保存；画布由宿主鉴权传送有界像素帧，缺失或损坏显示失败状态。导入成功后作为项目资产保留，文档取消或撤销不删除原件。后端已新增页面段落搜索及版本化 `.fcworld` 页面文档、首页和资产载荷，命令行回归通过；原生交互与跨平台往返仍待人工验收，不能写成 A3 全部退出。移动端 Markdown 行为保持；尚缺移动端页面编辑和 Windows WebView2 验收。旧正文转换层须待全部旧数据显式保存一次后删除。`npm run test:page-document` 当前覆盖 475 项通过。
- 2026-09-17：按 [ADR 0004](../docs/adr/0004-unified-object-layer-and-asset-scope.md) 增加阶段 A 退出条件：页面源码受控引用与组件实例序列化格式、统一对象层迁移、删除改走领域命令并记录真实删除者、节点身份保持与重映射规则、资产身份与归属范围分离、画布 `@layer` 与 macOS 最低版本确认。M2、M7 的现有实现（按记录存放派生文本、词条墓碑触发器、回执作用域 `CHECK`、项目绑定资产）待迁移，不能据此宣称 A2、A3 退出。
- 2026-09-18：统一对象层迁移已建立登记、受控引用、段落块、变更记录、保存回执和本地默认空间；领域删除记录真实删除者，直写 SQL 删除由触发器拒绝。桌面快照/CSV 侧载荷带页面文档与对象层，旧快照恢复保留仍存在词条的当前页面，revision 前进并使旧回执失效；新快照不再存储不回放的变更流与回执。资产身份与项目范围分离，原件按内容校验后迁往范围无关路径；桌面项目删除和世界包覆盖检查其余范围与引用后清理无主原件，旧路径副本在共享副本核验和登记切换后清理。Rust 校验生成段落块和引用，段落 FTS 已改为 trigram；长查询先截断索引候选，项目打开时重建缺块页面，旧 Markdown/旧页面重建前仍有搜索保底。`.fcworld` 新版本携带页面文档、首页、项目图片和对象层载荷，旧包继续导入；新包导入独立校验并重映射对象内链。两仓全量测试与前端当时的 447 项测试通过；独立范围撤销、中途崩溃后的孤儿文件重试、Android 快照实现、组件链授权和原生往返验收仍未关闭。
- 2026-09-18：阶段 A 剩余格式契约已由 [ADR 0005](../docs/adr/0005-page-document-node-identity-and-component-contract.md) 收口。九类节点身份保持/重映射规则均有可执行模型和测试，真实保存准备路径会补齐缺失 ID、修复后续重复 ID 并保留首个节点；引用式公共组件的定义字段、实例 HTML、`data-fc-part` 实例作用域及 `fc-component` 选择器边界已冻结。该改动对旧 v1 文档是加法，`DOCUMENT_VERSION` 保持 1。组件存储、宿主展开、编辑界面、组件链授权与世界包携带组件仍属于阶段 D；原生跨平台验收仍按 §6 执行。
