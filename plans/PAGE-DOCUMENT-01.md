...
- M7 桌面编辑页：第一至第四批 4a/4b 的页面代码与自动测试已接入，第三批的七项 macOS 人工验收已通过；第四批输入与输入法仍待三端原生验收。本批桌面正文入口只显示页面文档，浏览旧数据时临时转换为受管 HTML，编辑旧数据只产生草稿，首次显式保存获得 revision 1。桌面旧 Markdown 编辑、预览、工具栏、查找与大纲已移除；摘要与桌面词条字数/内容厚度改用页面派生文本，无文档时读取临时转换的纯文本。移动端原有 Markdown 行为保持，AI 旧正文写工具只对尚无页面文档的词条有效；已有页面文档时明确拒绝。后续顺序：桌面图片与内链入口 → 移动端页面编辑 → AI 语义化写入 → 资产管线。尚缺桌面插图和内链候选、移动端页面编辑、搜索/导出导入的页面文档接入、资产真实加载及 Windows WebView2 验收；旧正文转换层须待全部旧数据显式保存一次后删除。 `npm run test:page-document` 覆盖 66 个文件、395 项通过，Dock 布局 10 项通过；Rust 词条页面校验与 AI 写入护栏的回归通过，原生操作待人工验收。
