...
- M4 文档内核：移植完成（ca78378、aeee508）。当前 `npm run test:page-document` 随 M6 回归一起明确覆盖全部 43 个测试文件，共 288 项通过；领域导入闭包由测试约束为不依赖 React、Tauri、DOM 全局及反向业务模块。
- M5 草稿会话：部分完成（ca78378）。草稿、操作队列、冲突重试和历史模型已移植；生产 hooks 尚未移植，也未接入界面。
- M6 隔离画布：实现完成，待原生验收（55dc268、ff43dbf、6f97233、823ae5a、f66862c、475ab8a、25115db、7eef3b9；后续两次原生证据与启动时序修正见对应提交）。独立打包页面叠加严格 CSP，iframe 仅开放 `allow-scripts`；画布运行时构建为唯一的内联 IIFE，并以精确 SHA-256 同时满足画布 meta CSP 与 Tauri 页面专属响应头。第一次 macOS 调试构建原生验证证明 opaque-origin 画布的 `'self'` 不匹配 `tauri://localhost`，原有外部经典脚本和样式表会被两条 CSP 拒绝；当前构建已移除这些子资源，并避免静态 `style` 触发 nonce。第二次原生验证中 CSP 拒绝消失，但暴露内联脚本早于画布根节点启动；当前骨架把脚本放在根节点之后，并以 `DOMContentLoaded` 兜底。带 token、来源 Window、版本、序号和大小上限的 bridge 已接入只读渲染、选择、高亮、根内容尺寸与导航意图。宿主正常路径经 `previewCompiler` 与 guard 后下发，显式探针绕过时仍由画布独立校验。桌面和移动查看态入口及共享隔离探针只在 `VITE_PAGE_DOCUMENT_CANVAS=1` 构建出现，默认构建产物扫描无匹配。`npm run test:page-document` 覆盖 43 个文件、288 项通过；修正后的 macOS 加载、作者行内样式，以及 Windows WebView2、Android WebView 的 CSP、opaque origin、无网络与生命周期行为仍待原生产物验收。
