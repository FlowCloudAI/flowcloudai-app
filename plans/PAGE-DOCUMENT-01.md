...
- M2 core_world_data 存储切片：完成（0900e80、3d1b1ec）。词条与项目首页均有独立记录、revision CAS、幂等回执、派生文本和出链事务；回归覆盖项目首页创建、保存、重开、过期 revision，同键异内容拒绝，中途失败全回滚及每目标 64 条回执上限。`cargo test` 为 122 项通过；SQLite 最小特性链为 103 项通过。
- M3 Rust 校验与 Tauri command：完成当前页面文档存取切片（ca78378、0c4d3a1、c4b13f4）。`app_main` 使用 HTML5 解析树和 CSS token 校验源码，生成派生文本与出链目标；词条命令打开所属世界的词条库，项目首页命令打开项目库，错误保持结构化。`src-tauri/cargo test` 为 155 项通过、1 项联网测试忽略，其中页面文档契约集成测试 7 项通过。
- M4 文档内核：移植完成（ca78378、aeee508）。`npm run test:page-document` 明确覆盖全部 34 个测试文件，共 256 项通过；领域导入闭包由测试约束为不依赖 React、Tauri、DOM 全局及反向业务模块。
