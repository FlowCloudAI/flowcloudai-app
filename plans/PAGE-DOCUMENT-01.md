...
- 2026-09-18：统一对象层迁移已建立登记、受控引用、段落块、变更记录、保存回执和本地默认空间；领域删除记录真实删除者，直写 SQL 删除由触发器拒绝。桌面快照/CSV 侧载荷带页面文档与对象层，旧快照恢复保留仍存在词条的当前页面，revision 前进并使旧回执失效。资产身份与项目范围分离，原件按内容校验后迁往范围无关路径；Rust 校验生成段落块和引用，FTS 保留旧 Markdown/旧页面重建前的搜索保底。`.fcworld` 新版本携带页面文档、首页、项目图片和对象层载荷，旧包继续导入；新包导入独立校验并重映射对象内链。两仓全量测试与前端 446 项测试通过；Android 快照实现、组件链授权、节点身份规则和原生往返验收仍未关闭，阶段 A 不能据此宣布退出。
