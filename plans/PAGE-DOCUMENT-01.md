...
- M6 隔离画布：实现完成；macOS 原生验收通过（行内样式待补验），Android、Windows 待验证（55dc268、ff43dbf、6f97233、823ae5a、f66862c、475ab8a、25115db、7eef3b9；后续原生证据与修正见对应提交）。独立打包页面叠加严格 CSP，iframe 仅开放 `allow-scripts`；画布运行时构建为唯一的内联 IIFE，并以精确 SHA-256 同时满足画布 meta CSP 与 Tauri 页面专属响应头。macOS 调试构建已先后发现并修正 opaque-origin 下外链资源被 CSP 拒绝、内联脚本早于根节点启动、未分层运行时默认值压过作者主题三个问题。用户在 `1f28a56` 开关构建上完成五项复验：合法共享样例主题与结构正确，正常路径和跳过路径分别阻断恶意样例，链接不造成白屏或顶层跳转，画布高度稳定跟随内容，当前词条显示标题与 Markdown 降级正文。macOS 尚待验证作者行内 `style` 实际生效，并需用 Web Inspector 网络面板确认拦截时无外部请求。带 token、来源 Window、版本、序号和大小上限的 bridge 已接入只读渲染、选择、高亮、根内容尺寸与导航意图。宿主正常路径经 `previewCompiler` 与 guard 后下发，显式探针绕过时仍由画布独立校验。桌面和移动查看态入口及共享隔离探针只在 `VITE_PAGE_DOCUMENT_CANVAS=1` 构建出现，默认构建产物扫描无匹配。`npm run test:page-document` 覆盖 45 个文件、292 项通过；Android WebView 与 Windows WebView2 的 CSP、opaque origin、行内样式、无网络与生命周期行为仍待原生产物验收。
