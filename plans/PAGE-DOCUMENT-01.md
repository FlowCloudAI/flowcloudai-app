...
- M7 桌面编辑页：页面编辑、旧 Markdown 临时转换、桌面唯一页面正文入口及 AI 旧正文写入护栏已接入；第三批七项 macOS 人工验收已通过，第四批输入与输入法仍待三端原生验收。双链候选已由用户完成桌面人工验收。图片入口与项目范围授权已接入同一草稿、撤销和 revision 保存；画布由宿主鉴权传送有界像素帧，缺失或损坏显示失败状态。导入成功后作为项目资产保留，文档取消或撤销不删除原件。后端已新增页面段落搜索及版本化 `.fcworld` 页面文档、首页和资产载荷，命令行回归通过；原生交互与跨平台往返仍待人工验收，不能写成 A3 全部退出。移动端 Markdown 行为保持；尚缺移动端页面编辑、完整图像处理队列和 Windows WebView2 验收。旧正文转换层须待全部旧数据显式保存一次后删除。`npm run test:page-document` 当前覆盖 447 项通过。
