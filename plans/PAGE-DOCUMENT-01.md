...
- M7 桌面编辑页：页面编辑、旧 Markdown 临时转换、桌面唯一页面正文入口及 AI 旧正文写入护栏已接入；第三批七项 macOS 人工验收已通过，第四批输入与输入法仍待三端原生验收。双链候选已由用户完成桌面人工验收。本轮增加项目级稳定图片 ID、原件摘要与归属记录；桌面可导入或选择项目图片、经同一草稿内核插入和替换，并沿用已有 alt/图注编辑、撤销重做及 revision 保存。画布不加载资源 URL，而由宿主鉴权读取并传送有界像素帧；缺失或损坏显示失败状态。导入成功后即作为项目资产保留，文档取消或撤销不删除原件。上述图片链路已通过命令行测试，原生交互与三端显示仍待人工验收，不能写成 A3 全部退出。移动端 Markdown 行为保持；尚缺移动端页面编辑、搜索与 `.fcworld` 导出导入的页面文档及资产接入、完整图像处理队列和 Windows WebView2 验收。旧正文转换层须待全部旧数据显式保存一次后删除。本轮 `npm run test:page-document` 覆盖 73 个测试文件、428 项通过，Dock 布局 10 项通过。
