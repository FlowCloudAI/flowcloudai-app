...
- M4 文档内核：移植完成（ca78378、aeee508）。当前 `npm run test:page-document` 随编辑页第二批回归一起明确覆盖全部 53 个测试文件，共 320 项通过；领域导入闭包由测试约束为不依赖 React、Tauri、DOM 全局及反向业务模块。
...
- M6 隔离画布：实现完成；macOS、Android 原生验收通过，Windows 待验证（55dc268、ff43dbf、6f97233、823ae5a、f66862c、475ab8a、25115db、7eef3b9；后续原生证据与修正见对应提交）。独立打包页面叠加严格 CSP，iframe 仅开放 `allow-scripts`；画布运行时构建为唯一的内联 IIFE，并以精确 SHA-256 同时满足画布 meta CSP 与 Tauri 页面专属响应头。macOS 调试构建已先后发现并修正 opaque-origin 下外链资源被 CSP 拒绝、内联脚本早于根节点启动、未分层运行时默认值压过作者主题三个问题。用户随后在 macOS 开关构建和 Android aarch64 debug APK 上确认合法样例与行内样式正确，两道校验分别阻断恶意样例，Network 面板无外部请求，链接不造成白屏或顶层跳转，画布高度稳定跟随内容，后台恢复后仍可继续渲染，当前词条显示标题与 Markdown 降级正文；两端的 `window.origin === "null"` 与跨文档访问 `SecurityError` 也确认了 opaque origin。带 token、来源 Window、版本、序号和大小上限的 bridge 已接入只读渲染、选择、高亮、根内容尺寸与导航意图。宿主正常路径经 `previewCompiler` 与 guard 后下发，显式探针绕过时仍由画布独立校验。桌面和移动查看态入口及共享隔离探针只在 `VITE_PAGE_DOCUMENT_CANVAS=1` 构建出现，默认构建产物扫描无匹配。`npm run test:page-document` 当前覆盖 53 个文件、320 项通过；Windows WebView2 的 CSP、opaque origin、行内样式、无网络与生命周期行为仍待原生产物验收。
- M7 桌面编辑页：第二批完成（4750201、8079839、1b59fc4；属性 Dock 同批提交）。第一批已由用户在 macOS 开关调试构建中通过七项人工验收：进入页面模式、代码编辑保存与重开、中文输入、危险内容拦截、离开确认与切标签不弹、Markdown 不受影响、浏览模式预览 revision。第二批接入只读可视点选、画布与左栏图层双向选择，以及独占页面作用域 Dock 的所有宽度属性编辑；字号、字重、行高、外距、内距、容器间距、文字颜色与背景色均经文档内核写入同一草稿并进入统一历史。默认构建扫描继续排除入口、相关文案、CodeMirror 与页面属性内容。画布内输入、分宽度编辑、AI 分屏、拖拽、项目模板编辑与项目模板存储仍未实现。`npm run test:page-document` 覆盖 53 个文件、320 项通过，Dock 布局 9 项通过。
