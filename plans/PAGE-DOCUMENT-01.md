...
- M4 文档内核：移植完成（ca78378、aeee508）。当前 `npm run test:page-document` 随编辑页第三批回归一起明确覆盖全部 59 个测试文件，共 347 项通过；领域导入闭包由测试约束为不依赖 React、Tauri、DOM 全局及反向业务模块。
...
- M6 隔离画布：实现完成；macOS、Android 原生验收通过，Windows 待验证（55dc268、ff43dbf、6f97233、823ae5a、f66862c、475ab8a、25115db、7eef3b9；后续原生证据与修正见对应提交）。独立打包页面叠加严格 CSP，iframe 仅开放 `allow-scripts`；画布运行时构建为唯一的内联 IIFE，并以精确 SHA-256 同时满足画布 meta CSP 与 Tauri 页面专属响应头。macOS 调试构建已先后发现并修正 opaque-origin 下外链资源被 CSP 拒绝、内联脚本早于根节点启动、未分层运行时默认值压过作者主题三个问题。用户随后在 macOS 开关构建和 Android aarch64 debug APK 上确认合法样例与行内样式正确，两道校验分别阻断恶意样例，Network 面板无外部请求，链接不造成白屏或顶层跳转，画布高度稳定跟随内容，后台恢复后仍可继续渲染，当前词条显示标题与 Markdown 降级正文；两端的 `window.origin === "null"` 与跨文档访问 `SecurityError` 也确认了 opaque origin。带 token、来源 Window、版本、序号和大小上限的 bridge 已接入只读渲染、选择、高亮、根内容尺寸与导航意图。宿主正常路径经 `previewCompiler` 与 guard 后下发，显式探针绕过时仍由画布独立校验。桌面和移动查看态入口及共享隔离探针只在 `VITE_PAGE_DOCUMENT_CANVAS=1` 构建出现，默认构建产物扫描无匹配。`npm run test:page-document` 当前覆盖 59 个文件、347 项通过；Windows WebView2 的 CSP、opaque origin、行内样式、无网络与生命周期行为仍待原生产物验收。
- M7 桌面编辑页：第三批完成（4750201、8079839、1b59fc4、951108f、9b92082、58b38aa、cf3e011）。第一批已由用户在 macOS 开关调试构建中通过七项人工验收：进入页面模式、代码编辑保存与重开、中文输入、危险内容拦截、离开确认与切标签不弹、Markdown 不受影响、浏览模式预览 revision。第二批接入只读可视点选、画布与左栏双向选择，以及独占页面作用域 Dock 的所有宽度属性编辑。第三批落实页面模式的左栏取舍：左栏固定为组件树，左栏顶部“返回分类”按词条自身分类返回，无分类时回项目根；EntryEditor 自身顶栏返回保持原行为。组件树的展开和类型标识改用专用 SVG，不使用字符图标。属性 Dock 改为结构化调节控件：字号、行高和间距提供滑杆、数字、单位、步进及快捷值，外距和内距支持上下/左右联动，字重使用有限下拉，文字色与背景色提供主题色、标准色、最近颜色及更多颜色，所有修改与清除覆盖都经文档内核写入同一草稿和统一历史。新文档的 Markdown 降级段落直接带受管身份；已保存文档中可安全推断类型的不透明元素可逐个纳入，纳入只增加身份属性并可撤销，不提供批量纳入。图标规则由源码扫描固定：页面文档实现不得以 Unicode 箭头、技术符号、几何图形、装饰符号或项目符号冒充图标。默认构建扫描继续排除入口、相关文案、CodeMirror 与页面属性内容。画布内输入与输入法、分宽度编辑、AI 分屏、拖拽与 Grid 手柄、项目模板编辑与项目模板存储仍未实现；Windows WebView2 仍待原生验证。`npm run test:page-document` 覆盖 59 个文件、347 项通过，Dock 布局 9 项通过。
