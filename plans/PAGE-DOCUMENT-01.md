...
- 2026-09-18：统一对象层迁移已建立登记、受控引用、段落块、变更记录、保存回执和本地默认空间；领域删除记录真实删除者，直写 SQL 删除由触发器拒绝。桌面快照/CSV 侧载荷带页面文档与对象层，旧快照恢复保留仍存在词条的当前页面，revision 前进并使旧回执失效；新快照不再存储不回放的变更流与回执。资产身份与项目范围分离，原件按内容校验后迁往范围无关路径；桌面项目删除检查其余范围和引用后清理无主原件，旧路径副本在共享副本核验和登记切换后清理。Rust 校验生成段落块和引用，段落 FTS 已改为 trigram；长查询先截断索引候选，项目打开时重建缺块页面，旧 Markdown/旧页面重建前仍有搜索保底。`.fcworld` 新版本携带页面文档、首页、项目图片和对象层载荷，旧包继续导入；新包导入独立校验并重映射对象内链。两仓全量测试与前端 447 项测试通过；世界包覆盖后的资产磁盘清理、Android 快照实现、组件链授权、节点身份规则和原生往返验收仍未关闭，阶段 A 不能据此宣布退出。
