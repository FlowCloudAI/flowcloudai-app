...
受管预览像素，不含脚本或外部资源。运行时按单页默认资产块上限缓存最近 100 项压缩图片及失败状态，宿主缓存最近
16 项授权帧或失败结果并合并在途读取；两级缓存都随会话或项目切换释放。
宿主缓存的 effect 清理延迟至下一任务：React StrictMode 与 Fast Refresh 在同一任务内的 cleanup/setup 不会销毁仍在使用的缓存，
真正离开时旧会话不可再接收回执。命中缓存的图片重渲染时直接赋给新 `<img>`，失败也直接恢复上次状态，
均不再向后端读取；重新打开文档或切换项目才重试。CSS 中仅被校验并清空、并未显示的 `fcasset` 引用
不请求帧。加载中、不可用与内容无效由运行时固定文案的 `data:` 状态图显示在原 `<img>` 上，
不插入兄弟节点；状态因此跟随原图的 `hidden`、作者 `display`、层叠和变换。成功后删除隔离占位标记。
...
与画布坐标系中的有界矩形；离开锚点时回报空状态。宿主把矩形换算到页面坐标，仅在词条浏览模式
...
及透明度均按原 `<img>` 绘制；开发模式的 StrictMode 重放后图片仍可读取；`hidden` 与作者
`display: none` 对加载、缺失和格式无效状态同样生效；含损坏图片时连续输入不闪回加载状态、
不重复读取后端；插入、替换、缺失、无权与格式损坏分别得到正确状态。Node 测试和静态浏览器截图
即使通过，也不能代替这些打包 WebView 结果。
