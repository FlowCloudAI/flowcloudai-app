...
test('只有桌面浏览入口接入悬停浮窗，编辑模式与移动端不接入', () => {
    ...
    const sourceWorkspace = readFileSync(new URL('../../components/source/SourceWorkspace.tsx', import.meta.url), 'utf8')
    const mobile = readFileSync(new URL('../../../../app/mobile/pages/MobileEntryDetailView.tsx', import.meta.url), 'utf8')
    assert.equal(entry.match(/onLinkHover=\{handlePageDocumentLinkHover\}/gu)?.length, 1)
    assert.match(entry, /<EntryEditorLinkPreview/u)
    assert.match(entry, /if \(!active \|\| editorMode !== 'browse'\)/u)
    assert.doesNotMatch(editor, /onLinkHover/u)
    assert.doesNotMatch(sourceWorkspace, /onLinkHover/u)
