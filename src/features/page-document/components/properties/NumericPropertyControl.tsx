...
export function NumericPropertyControl({field, compact = false, embedded = false, presets, onChange}: NumericPropertyControlProps) {
    const baseDefinition = numericPropertyDefinition(field.property)
    if (!baseDefinition) throw new TypeError(`属性 ${field.property} 没有数值控件定义。`)
    const definition = presets ? {...baseDefinition, presets} : baseDefinition
    ...
    const controls = <>
        ...
        {!embedded && field.localValue !== null && (
            <Button
                aria-label="清除"
        ...
        {(error || (!embedded && field.reason)) && <p className="page-document-property__message" role={error ? 'alert' : undefined}>{error ?? field.reason}</p>}
    </>
    if (embedded) {
        return <div className={`page-document-property__embedded-numeric${compact ? ' is-compact' : ''}`}>
            {controls}
        </div>
    }
    return (
