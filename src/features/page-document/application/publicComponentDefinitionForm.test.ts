...
describe('公共组件创建表单', () => {
    it('把属性与插槽逐行格式转换为创建命令输入', () => {
        const input = buildPublicComponentCreationInput(PROJECT_ID, COMPONENT_ID, {
            name: ' 信息卡片 ',
            category: ' 基础 ',
            ...
            styleVariableLines: '--accent | <color> | red',
        })
        assert.equal(input.componentId, COMPONENT_ID)
        ...
        assert.deepEqual(input.styleVariableSchema, [{name: '--accent', syntax: '<color>', initialValue: 'red'}])
    })

    it('拒绝重复名称与含糊的必填标记', () => {
        assert.throws(() => buildPublicComponentCreationInput(PROJECT_ID, COMPONENT_ID, {
            name: '卡片',
            category: '基础',
