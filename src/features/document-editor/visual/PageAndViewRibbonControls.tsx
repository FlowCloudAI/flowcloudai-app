...
export function ResponsiveEditContextControl({
    value,
    onChange,
    compact = false,
}: {
    value: ResponsiveViewportContext
    onChange: (context: ResponsiveViewportContext) => void
    compact?: boolean
}) {
    return <div
        className={`document-ribbon-choice-row document-responsive-edit-context${compact ? ' is-compact' : ''}`}
        role="group"
        aria-label="修改生效范围"
    >
        {(Object.keys(RESPONSIVE_EDIT_SCOPE_LABELS) as ResponsiveViewportContext[]).map(context => <button
            aria-pressed={value === context}
            key={context}
            onClick={() => onChange(context)}
            title={RESPONSIVE_EDIT_SCOPE_NOTES[context]}
            type="button"
        >
            {RESPONSIVE_EDIT_SCOPE_LABELS[context]}
        </button>)}
    </div>
}
...
export function ViewRibbonControls({previewMode, editContext, outlineVisible, layoutGuidesVisible, onPreviewModeChange, onEditContextChange, onOutlineVisibleChange, onLayoutGuidesVisibleChange, onOpenDisplay, onOpenDeveloperInfo}: {
    ...
    return <>
        ...
        <DocumentRibbonGroup label="修改生效范围" priority="high" wide>
            <ResponsiveEditContextControl onChange={onEditContextChange} value={editContext} />
