...
8. **底部面板（`MobileBottomSheet`）只承载「一屏放得下、且不需要输入」的内容。** 命中下列任意一条，就改用页面栈里的完整页面，不要塞进面板：
   - 内容会超出面板高度、需要在面板内滚动；
   - 内容包含文本框、数字输入、可搜索下拉等任何会唤起键盘的控件。

   理由不是观感，是三个机制会互相打架：

   - **滚动与关闭手势争夺同一个下拉。** 面板靠向下拖拽关闭，内部可滚动区域也要消费向下拖拽。`MobileBottomSheet.tsx` 的 `shouldSkipBottomSheetDrag` 就是这个仲裁打的补丁——它遍历祖先链找可滚动容器、并对 `input / textarea / select / contenteditable` 直接放弃拖拽。**那段防御是给边界情况兜底的，不是「可以往里塞滚动和输入」的许可。**
   - **键盘遮挡量、面板高度、拖拽偏移抢同一段垂直空间。** 按 §11 平台契约，软键盘布局同一时刻只能有一个 owner；面板自身的 `max-height` 和 `translateY` 会成为第二个 owner。
   - **面板没有真正的返回栈。** 面板里再开面板或二级内容，返回键要吃两次以上——这是 2026-08 移动端词条编辑挨骂的原始形态之一。

   完整页面没有这三个问题：自己的滚动容器、自己的顶栏、页面栈提供的边缘返回手势与返回键处理。
...
6. 所有可点元素命中区 ≥ 48。
7. 正文对比度 ≥ 4.5:1。
8. 底部面板内没有滚动内容与输入控件（§9 规则 8）。该项无法用 grep 断定（内容经 `children` 传入），必须人工确认。
